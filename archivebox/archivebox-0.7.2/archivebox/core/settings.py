...
if LDAP:
    try:
        ...
        assert (
            AUTH_LDAP_SERVER_URI and LDAP_USERNAME_ATTR and LDAP_USER_FILTER
        ...
        sys.stderr.write(
            "[X] Error: Found LDAP=True config but LDAP packages not installed. You may need to run: pip install archivebox[ldap]\n\n"
        )
...
TIME_ZONE = (
    TIMEZONE  # django convention is TIME_ZONE, archivebox config uses TIMEZONE, they are equivalent
)
...
class NoisyRequestsFilter(logging.Filter):
    def filter(self, record):
        ...
        for ignorable_url_pattern in IGNORABLE_404_URLS:
            ignorable_log_pattern = re.compile(
                f'^"GET /.*/?{ignorable_url_pattern.pattern[:-1]} HTTP/.*" (200|30.|404) .+$',
                re.I | re.M,
            )
        ...
        ignoreable_200_log_pattern = re.compile(
            r'"GET /static/.* HTTP/.*" (200|30.) .+', re.I | re.M
        )
