...
def merge_links(a: Link, b: Link) -> Link:
    ...
    ), f"Cannot merge two links with different URLs ({a.base_url} != {b.base_url})"
    ...
    timestamp = a.timestamp if float(a.timestamp or 0) < float(b.timestamp or 0) else b.timestamp

    # all unique, truthy tags
    tags_set = set(tag.strip() for tag in (a.tags or "").split(",")) | set(
    ...
    for method in all_methods:
        deduped_jsons = {
            pyjson.dumps(result, sort_keys=True, cls=ExtendedEncoder) for result in history[method]
        }
        history[method] = list(
            reversed(
                sorted(
...
def write_main_index(links: List[Link], out_dir: Path = OUTPUT_DIR) -> None:
    ...
    try:
        with timed_index_update(out_dir / SQL_INDEX_FILENAME):
            ...
            os.chmod(
                out_dir / SQL_INDEX_FILENAME, int(OUTPUT_PERMISSIONS, base=8)
            )  # set here because we don't write it with atomic writes
...
@enforce_types
def dedupe_links(snapshots: QuerySet, new_links: List[Link]) -> List[Link]:
    """
    ...
    new_links = [link for link in new_links if not snapshots.filter(url=link.url).exists()]
...
@enforce_types
def write_link_details(
    link: Link, out_dir: Optional[str] = None, skip_sql_index: bool = False
) -> None:
...
LINK_FILTERS = {
    ...
    "domain": lambda pattern: Q(url__istartswith=f"http://{pattern}")
...
@enforce_types
def q_filter(
    snapshots: QuerySet, filter_patterns: List[str], filter_type: str = "exact"
) -> QuerySet:
    q_filter = Q()
...
def search_filter(
    snapshots: QuerySet, filter_patterns: List[str], filter_type: str = "search"
) -> QuerySet:
...
def get_valid_folders(snapshots, out_dir: Path = OUTPUT_DIR) -> Dict[str, Optional[Link]]:
    ...
    links = [snapshot.as_link_with_details() for snapshot in snapshots.iterator()]
    return {link.link_dir: link for link in filter(is_valid, links)}
