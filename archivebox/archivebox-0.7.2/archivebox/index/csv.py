...
def links_to_csv(
    ...
    row_strs = (link.to_csv(cols=cols, ljust=ljust, separator=separator) for link in links)
...
def to_csv(obj: Any, cols: List[str], separator: str = ",", ljust: int = 0) -> str:
    ...
    return separator.join(to_json(getattr(obj, col), indent=None).ljust(ljust) for col in cols)
