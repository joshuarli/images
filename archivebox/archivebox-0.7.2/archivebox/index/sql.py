...
def parse_sql_main_index(out_dir: Path = OUTPUT_DIR) -> Iterator[Link]:
    ...
    return (Link.from_json(page.as_json(*Snapshot.keys)) for page in Snapshot.objects.all())


@enforce_types
def remove_from_sql_main_index(
...
def write_link_to_sql_index(link: Link):
    ...
    tag_list = list(
        dict.fromkeys(tag.strip() for tag in re.split(TAG_SEPARATOR_PATTERN, link.tags or ""))
    )
    info.pop("tags")
