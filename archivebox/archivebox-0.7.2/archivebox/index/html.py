        ...
        LINK_DETAILS_TEMPLATE,
        {
            ...
            **link_info["canonical"],
            ...
            "archive_url": urlencode(
                wget_output_path(link) or (link.domain if link.is_archived else "")
            )
            or "about:blank",
...
def snapshot_icons(snapshot) -> str:
    ...
    def calc_snapshot_icons():
        ...
        archive_results = snapshot.archiveresult_set.filter(
            status="succeeded", output__isnull=False
        )
        ...
        for extractor, _ in EXTRACTORS:
            if extractor not in exclude:
                existing = (
                    extractor_outputs[extractor]
                    and extractor_outputs[extractor].status == "succeeded"
                    and extractor_outputs[extractor].output
                )
            ...
            if extractor == "archive_org":
                ...
                # get from db (faster)
                exists = (
                    extractor in extractor_outputs
                    and extractor_outputs[extractor]
                    and extractor_outputs[extractor].status == "succeeded"
                    and extractor_outputs[extractor].output
                )
                ...
                output += '<a href="{}" class="exists-{}" title="{}">{}</a> '.format(
                    canon["archive_org_path"],
                    str(exists),
                    "archive_org",
                    icons.get("archive_org", "?"),
                )

        result = format_html(
            '<span class="files-icons" style="font-size: 1.1em; opacity: 0.8; min-width: 240px; display: inline-block">{}<span>',
            mark_safe(output),
        )
