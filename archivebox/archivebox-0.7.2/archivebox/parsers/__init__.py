...
@enforce_types
def parse_links(
    source_file: str, root_url: Optional[str] = None, parser: str = "auto"
) -> Tuple[List[Link], str]:
...
def save_text_as_source(
    raw_text: str, filename: str = "{ts}-stdin.txt", out_dir: Path = OUTPUT_DIR
) -> str:
...
@enforce_types
def save_file_as_source(
    path: str,
    timeout: int = TIMEOUT,
    filename: str = "{ts}-{basename}.txt",
    out_dir: Path = OUTPUT_DIR,
) -> str:
    """download a given url's content into output/sources/domain-<timestamp>.txt"""
    ts = str(datetime.now(timezone.utc).timestamp()).split(".", 1)[0]
    source_path = str(
        OUTPUT_DIR / SOURCES_DIR_NAME / filename.format(basename=basename(path), ts=ts)
    )
...
for url_str, num_urls in _test_url_strs.items():
    assert (
        len(re.findall(URL_REGEX, url_str)) == num_urls
