...
class Pocket(object):
    ...
    api_endpoints = dict(
        (method, "https://getpocket.com/v3/%s" % method) for method in "add,send,get".split(",")
    )
    ...
    def _make_request(cls, url, payload, headers=None):
        ...
        if r.status_code > 399:
            ...
            raise EXCEPTIONS.get(r.status_code, PocketException)("%s. %s" % (error_msg, extra_info))
    ...
    def auth(
        ...
        raw_input(
            "Please open %s in your browser to authorize the app and " "press enter:" % auth_url
        )
