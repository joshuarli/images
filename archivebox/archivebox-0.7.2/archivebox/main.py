...
""".format(
                VERSION, out_dir, COMMANDS_HELP_TEXT, **ANSI
            )
        )
...
def version(quiet: bool = False, out_dir: Path = OUTPUT_DIR) -> None:
    ...
    if not quiet:
        ...
        OUTPUT_IS_REMOTE_FS = (
            DATA_LOCATIONS["OUTPUT_DIR"]["is_mount"] or DATA_LOCATIONS["ARCHIVE_DIR"]["is_mount"]
        )
        print(
...
def init(
    ...
    if (ADMIN_USERNAME and ADMIN_PASSWORD) and not User.objects.filter(
        username=ADMIN_USERNAME
    ).exists():
        print(
            "{green}[+] Found ADMIN_USERNAME and ADMIN_PASSWORD configuration options, creating new admin user.{reset}".format(
                **ANSI
            )
        )
...
def add(
    ...
    tags = [
        Tag.objects.get_or_create(name=name.strip())[0] for name in tag.split(",") if name.strip()
    ]
    if tags:
    ...
    if CAN_UPGRADE:
        hint(
            f"There's a new version of ArchiveBox available! Your current version is {VERSION}. You can upgrade to {VERSIONS_AVAILABLE['recommended_version']['tag_name']} ({VERSIONS_AVAILABLE['recommended_version']['html_url']}). For more on how to upgrade: https://github.com/ArchiveBox/ArchiveBox/wiki/Upgrading-or-Merging-Archives\n"
        )
...
def schedule(
    ...
    if every or add:
        ...
        print(
            "\n".join(
                f"  > {cmd}" if str(cmd) == str(new_job) else f"    {cmd}" for cmd in existing_jobs
            )
        )
