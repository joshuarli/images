...
def atomic_write(
    ...
    try:
        ...
        if ENFORCE_ATOMIC_WRITES:
            ...
            print(
                "    You can store the archive/ subfolder on a hard drive or network share that doesn't support support syncronous writes,"
            )
            print(
...
def chmod_file(path: str, cwd: str = ".") -> None:
    ...
    if not root.exists():
        raise Exception(
            "Failed to chmod: {} does not exist (did the previous step fail?)".format(path)
        )
