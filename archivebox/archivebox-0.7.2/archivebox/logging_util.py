...
def get_fd_info(fd) -> Dict[str, Any]:
    ...
    return {
        ...
        "IS_LINE_BUFFERED": IS_LINE_BUFFERED,
...
def reject_stdin(caller: str, stdin: Optional[IO] = sys.stdin) -> None:
    ...
    if not stdin.isatty():
        ...
        if stdin_raw_text.strip():
            ...
            stderr(
                f'[!] The "{caller}" command does not accept stdin (ignoring).',
                color="red",
            )
...
def progress_bar(seconds: int, prefix: str = "") -> None:
    ...
    try:
        for s in range(seconds * chunks):
            ...
            log_pct = (
                log(pct_complete or 1, 10) / 2
            ) * 100  # everyone likes faster progress bars ;)
            bar_width = round(log_pct / (100 / chunks))
            last_width = max_width

            # ████████████████████           0.9% (1/60sec)
            sys.stdout.write(
                "\r{0}{1}{2}{3} {4}% ({5}/{6}sec)".format(
                    prefix,
...
def log_cli_command(subcommand: str, subcommand_args: List[str], stdin: Optional[str], pwd: str):
    ...
    stderr(
        "{black}[i] [{now}] ArchiveBox v{VERSION}: {cmd}{reset}".format(
...
def log_importing_started(urls: Union[str, List[str]], depth: int, index_only: bool):
    ...
    print(
        "{green}[+] [{}] Adding {} links to index (crawl depth={}){}...{reset}".format(
...
def log_source_saved(source_file: str):
    print(
        "    > Saved verbatim input to {}/{}".format(
            SOURCES_DIR_NAME, source_file.rsplit("/", 1)[-1]
        )
    )
...
def log_crawl_started(new_links):
    print()
    print(
        "{green}[*] Starting crawl of {} sites 1 hop out from starting point{reset}".format(
            len(new_links), **ANSI
        )
    )
...
def log_indexing_process_started(num_links: int):
    ...
    print()
    print(
        "{black}[*] [{}] Writing {} links to main index...{reset}".format(
...
def log_archiving_started(num_links: int, resume: Optional[float] = None):
    ...
    if resume:
        print(
            "{green}[▶] [{}] Resuming archive updating for {} pages starting from {}...{reset}".format(
                start_ts.strftime("%Y-%m-%d %H:%M:%S"),
                num_links,
                resume,
                **ANSI,
        ...
        print(
            "{green}[▶] [{}] Starting archiving of {} snapshots in index...{reset}".format(
                start_ts.strftime("%Y-%m-%d %H:%M:%S"),
                num_links,
                **ANSI,
            )
...
def log_archiving_finished(num_links: int):
    ...
    print()
    print(
        "{}[√] [{}] Update of {} pages complete ({}){}".format(
            ANSI["green"],
...
def log_link_archiving_finished(
    link: "Link", link_dir: str, is_new: bool, stats: dict, start_ts: datetime
):
    ...
    print(
        "        {black}{} files ({}) in {}s {reset}".format(
            size[2], printable_filesize(size[0]), duration, **ANSI
        )
    )
...
def log_archive_method_finished(result: "ArchiveResult"):
    ...
    quoted_cmd = " ".join(
        '"{}"'.format(arg) if (" " in arg) or (":" in arg) else arg for arg in result.cmd
    )
...
def log_list_started(filter_patterns: Optional[List[str]], filter_type: str):
    print(
        "{green}[*] Finding links in the archive index matching these {} patterns:{reset}".format(
            filter_type,
            **ANSI,
        )
    )
...
def log_removal_finished(all_links: int, to_remove: int):
    if all_links == 0:
        ...
        print()
        print(
            "{red}[√] Removed {} out of {} links from the archive index.{reset}".format(
                to_remove,
                all_links,
                **ANSI,
            )
...
def printable_folders(folders: Dict[str, Optional["Link"]], with_headers: bool = False) -> str:
    return "\n".join(
        f'{folder} {link and link.url} "{link and link.title}"' for folder, link in folders.items()
    )
